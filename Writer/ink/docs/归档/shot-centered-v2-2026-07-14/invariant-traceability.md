# 测试与审计追踪矩阵（Shot 中心历史基线）

> **状态**：v1 旧实现追踪基线；Scene-first 追踪矩阵待迁移
> **定位**：把旧 bugfix、架构决策和新生产能力映射到新系统的不变量、测试与阻断里程碑。没有映射的能力不得视为完成。
>
> **优先级声明**：本表中的 `writing_shot_*`、每 shot current revision 等不变量只描述现有实现和迁移保护项。目标架构必须新增 Scene Revision、Chapter Candidate Branch、Chapter Snapshot、AI 防原地修改和 Scene 契约四层的不变量；详见 [scene-first-authority-amendment.md](scene-first-authority-amendment.md)。
