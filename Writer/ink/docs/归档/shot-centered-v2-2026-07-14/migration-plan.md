# 实施计划 — Pre-M0 + M0-M6 从 0 构建完整生产版（Shot 中心历史基线）

> **状态**：v4（2026-07-05，本地 baseline 验收完成），对应 `design-v2.md`
> **定位**：**从 0 构建完整生产版**，不基于旧 `inkflow/` 改造。无工期、无双轨并行、无旧系统迁移、无 MVP 裁剪。
>
> **2026-07-14 修正**：原 M0-M6 shot 中心实现已成为 Scene-first 迁移来源。当前最高优先任务是落实 [scene-first-authority-amendment.md](scene-first-authority-amendment.md)，将正式权威原子升级为 Scene；旧 `writing_shot_*` 表不得与新 Scene 权威链并行充当真相源。

## Scene-first 迁移前置门禁

1. 新增 Scene、Scene Contract、Scene Revision、Chapter Candidate Branch 和 Chapter Snapshot 权威实体。
2. 内部 shot 降级为 Scene Revision 的可选工作切片，不再独立 accepted/封版。
3. 正文读取与导出改为 active Chapter Snapshot，不再动态拼接 shot current revision。
4. 第9—14步的“稿件”必须是完整章节候选分支，选优入口为不少于3篇有效候选、具备实质差异且至少一篇达到文学绝对门槛。
5. AI 写入必须创建不可变 Scene Revision，并使用预期父版本校验；激活与生成分离。
6. Scene-first 数据迁移、回归测试和真实章节试点完成前，不得宣称新流程已投产。
...
- 写作质量是硬门禁：目标架构中 scene/chapter/book 任一层未达标，不得 accepted/export；迁移前 shot gate 只作为当前实现基线，human accept 不得覆盖硬失败。
