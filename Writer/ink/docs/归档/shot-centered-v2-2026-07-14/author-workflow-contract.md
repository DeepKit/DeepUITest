# 作者工作流契约 — 完整生产版（Shot 中心历史基线）

> **状态**：v1 旧工作流基线；2026-07-14 Scene-first 修正案已生效
> **定位**：Ink v2 的用户可执行契约。技术实现必须服务这里定义的完整工作闭环，不存在 MVP 裁剪版。
>
> **强制修正**：以 [scene-first-authority-amendment.md](scene-first-authority-amendment.md) 为准。本文中“shot 级正式封版、逐 shot 选优、shot accepted canonical”等旧表述只描述迁移前实现；目标工作流以 Scene 为最小权威单位、完整 Chapter Candidate 为文学选优单位、Chapter Snapshot 为正式导出单位。
...
- 所有正式正文必须通过 scene/chapter/book 三层质量硬门禁；内部 shot 检查不得替代 Scene 整体评审；人工 accept 不得覆盖硬质量失败。
...
| `ink setup --chapter N` | 生成/更新章节节奏、Scene 契约、大纲候选 | chapter specs、scene contracts、outline specs、task cards |
...
- 质量硬门禁：目标读者、文体标杆、正/反例、禁用俗套、水文模式、scene/chapter/book 阈值、盲评策略、继续阅读目标、protected_roughness
- writer model pool 与 jury model pool

`setup --chapter N` 必须生成：

- 章节节奏契约
- 每个 Scene 的 hard constraints、source DNA、soft goals、creative openings、entry/exit state；内部 shot 卡仅作非权威执行切片
- 大纲候选与 drift_score
...
2. **前 6 章灰度生成**：在全书基线封板后生成前 6 章，用来验证契约抽取、章/Scene 细化、质量门禁、恢复点和人类交互负担。
3. **局部作用域修订**：后续按卷/部/章/Scene 开 `ScopedDecisionSession`。局部修订只能细化或覆盖该作用域内的契约，不得反向修改全书红线。受影响下游 prompt、draft、review 必须标记 stale。

## 4. Write

`write --chapter N` 是完整生产流水线：

1. 从 DB reload active Scene Contract、事实快照和章节候选分支上下文。
2. 生成完整 Chapter Candidate Branch；内部可按 shot 分段生成，但正式候选必须绑定完整 Scene Revision 序列。
...
10. Scene seal 后不得原地回退；后续问题通过 revise/reject 创建新 Scene Revision 和新章节候选分支。
...
- 每个 Scene 候选的淘汰原因、hard gate、jury 维度分，以及内部 shot 诊断信息
...
- 全部必要 Scene 已通过 Scene 级硬门并绑定到当前 Chapter Candidate Branch
- 每个入选 Scene Revision 已通过质量门；内部 shot 局部修订已经重新合成为完整 Scene Revision 并复审
...
验收通过标准：无 degraded 假通过、无低于 scene/chapter/book 质量硬门禁的文本进入 accepted/export、无盲评/继续阅读失败文本进入 accepted、无 productive_deviation 被 polish 磨平、无人工 override 硬质量失败、无未审稿正文进入导出、未确认 DecisionSession 不进入 prompt、source hash 变化阻断旧确认、所有 AI 调用可追溯、所有人工决策可追溯、所有旧关键不变量在 `invariant-traceability.md` 中有对应测试。
