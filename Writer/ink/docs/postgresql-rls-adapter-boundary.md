# PostgreSQL / RLS Adapter Boundary

> **Status**: planning boundary for a future PostgreSQL migration. The current runtime remains SQLite.
...
A future PostgreSQL adapter must preserve these runtime semantics before it can replace SQLite:
...
Before enabling a PostgreSQL backend, add tests that cover:
