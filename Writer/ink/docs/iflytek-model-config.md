...
| 模型 ID | 家族 | 类型 | 备注 |
|---|---|---|---|
| `xopglm52` | 智谱 GLM-5.2 | 标准 | **写作主模型**，质量/速度均衡 |
| `xopglm51` | 智谱 GLM-5.1 | 标准 | |
| `xopglm5` | 智谱 GLM-5 | 标准 | |
...
| `xopqwen36v35b` | 阿里 Qwen-3.6-35B | 标准 | 最快（~0.5s） |
| `xopglmv47flash` | 智谱 GLM-4.7-Flash | 标准 | |
| `xop3qwencodernext` | 阿里 Qwen-Coder-Next | 标准 | 代码模型，写作弱 |
...
```json
["xopglm52", "xopdeepseekv4pro", "xopkimik26"]
```

- **3 个不同家族**（智谱 / DeepSeek / 月之暗面），保证多样性
- 都是标准模型，content 直接产出
...
```json
["xopglm52", "xopdeepseekv4pro", "xopqwen36v35b", "xopkimik26", "xminimaxm25"]
```

- **5 个家族**，DB CHECK 要求 `>= jury_model_pool_min`（默认 3）
- `xminimaxm25`（推理模型）做深度审查，需要时单独调高 `max_tokens`
...
SET writer_model_pool = '["xopglm52","xopdeepseekv4pro","xopkimik26"]',
    jury_model_pool = '["xopglm52","xopdeepseekv4pro","xopqwen36v35b","xopkimik26","xminimaxm25"]'
