...
> **状态**：当前生产内核开发队列已完成本地验证。主编台、DecisionSession、源文档规范化、选择式对话协议、source coverage 策略已入文档，待进入下一批开发。
> **最后更新**：2026-07-06
...
- **source coverage gate**：实现原子条款 × contract field 覆盖矩阵；必填字段没有 confirmed clause 或人工空值理由时，阻断 BookContract/局部契约确认。
- **双模型抽取完整性检查**：实现 primary/crosscheck 抽取 run，对不一致、漏抽、冲突和低置信项生成 coverage gap/conflict。
- **过程文件处理**：将 `better.md` 作为 `process_scratch` 输入；抽取并合并完成后清空，记录 processed manifest/hash、抽取条款 ID、contract patch ID、DecisionSession ID，禁止后续 prompt/contract 直接引用已处理过程文件。
