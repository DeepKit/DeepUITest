...
硬规则裁判不处理风格项清零。段落长度、方言点缀、感官密度、身体时刻开场进入风格/文学评价或导出版式优化；远端 LLM 若把这些问题作为 hard-rule 低分，只能作为 advisory 记录，不能使候选稿 `eligible=False`。`characters_alive` 表示角色存活状态，不是唯一允许出场名单。

每章生产前，`setup --chapter` 和 `run --chapter` 必须确认 setup 包与当前元契约一致。契约仍含“只生成第 N 章”或旧段落锁时，属于契约未校准，生产前失败；这类问题不能留给远端 jury 在评分阶段发现。

阈值内部统一为 0-100；配置允许写 10 分制（如 `8.5` 自动换算为 `85`）。默认要求至少 2 个候选稿达到 Y，避免“矮子里拔高个”。

---

## 1.2 v3.22 Contract-first 资格先于 PK 生产线

v3.16 分层裁判解决了“硬规则、类型职责、文学评分不能混合平均”的问题，但真实第 3 章暴露出更上游的缺陷：契约确认缺少复审、大纲门禁没有把契约违规挡在写作前，赛马评估也没有先判断候选稿是否有资格参赛。因此 v3.22 的权威顺序改为：

```text
Contract Draft
  → Contract Auditor Round 1: 结构完整性复审
  → Contract Auditor Round 2: 生产就绪复审
  → 两轮通过后 confirmed；不通过则打回架构师和人类继续讨论
  → Setup 编译 chapter hard fact pack:
...
人类在每次审查后可选择：
- **接受报告**：架构师修改 → 重新编译 → 再审查（循环）
- **部分接受**：只改 P0/P1
- **跳过**：直接确认 → 进入下一级

叙事分析师**不修改契约**——只输出诊断报告。规范文档：`narrative-analyst.md`
