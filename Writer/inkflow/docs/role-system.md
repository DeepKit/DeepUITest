...
| # | 职责 | 触发时机 | 输出 |
|---|------|---------|------|
| 1 | 审查世界观契约（L1） | 架构师编译 L1 完成 | 诊断报告 |
| 2 | 审查角色弧线契约（L2） | 架构师编译 L2 完成 | 诊断报告 |
| 3 | 审查章节结构契约（L3） | 架构师编译 L3 完成 | 诊断报告 |
| 4 | 审查 Shot 级契约（L4） | 架构师编译 L4 完成 | 诊断报告 |
...
| 弧线完整性 | Vogler / Truby |
| 铺垫与回收 | Chekhov |
| 信息差分布 | 三个不对称（信息/时间/后果） |
| 节奏曲线 | 场景姿态（ASTO） |
| 视角切换 | 叙事学 |
| 主题一致性 | Barthes 五码 |
| 结构债务 | 系统化 |

**世界设定检查**（5 项）：规则自洽 / 承重能力 / 传感器盲区 / 放大效应 / 非恶意原则

**悬疑设计检查**（5 项）：信息不对称 α / 时间不对称 β / 后果不对称 γ / 章末钩子 / 伏笔密度
...
1. `ink init`：与 AI 架构师沟通全书目标 → AI 架构师编译章以上层级契约草稿 → 人类审核继承摘要并继续讨论 → 架构师修订草案 → `ink confirm-contract` 触发契约审计师两轮复审；通过后才 confirmed 入库，不通过则打回架构师和人类继续讨论。
...
**量程**：每维度 0-100 分，整数分；默认灯色阈值为 green ≥ 85 / yellow ≥ 65 / red < 65。
...
当前实现默认使用 3 个评委模型和 5 个 v4 评分维度：`contract_compliance`、`forbidden_expression`、`reading_fluency`、`suspense_effectiveness`、`unexpected_value`。旧 6 基础裁判 + 0-3 动态裁判是历史设计口径，作为兼容枚举保留。
...
红黄绿阈值由项目配置控制。标准 shot 使用 `trimmed_mean`；留白 shot 使用 CREATIVE-3 的 `creative_score` 权重选稿。
...
17. 多模型裁判团评分（标准 shot 用 trimmed mean；留白 shot 用 creative_score）。
