...
## v3.26 第 3 章重复片段诊断与门禁补强 (2026-07-02)

人工审读《白灯法则》`v01.c03` 时指出 `雨季`、`常规运输条件下的密封`、`许怀山回到工厂` 三个片段雷同。本轮对照导出正文、shot contract、候选 draft、L4 结果和生成时间线，确认问题主要在生产环节，契约也存在场景锚点不够结构化的弱点。

### 诊断

- meta-contract 中三条事件顺序是清楚的：s01 转运站见军列；s02 前线露天堆放数日后出现微裂纹且批号无法确认；s03 回工厂车间门口闻到异常气味。
- s02 生成过多条正确候选（“雨停了三天 / 露天堆场 / 微裂纹 / 批号不清”），但最终 winner 选中了高分重复稿。
- s03 生成时间早于 s02 最终重写；s02 后续被替换后，s03 没有因上游 revision 更新而失效，导出时带着旧上下文。

### 修复

- L4 新增 `contract_scene` 检查：契约明确要求露天堆放、微裂纹、批号不清、回到工厂、车间门口、异常气味时，正文和开头必须落地对应锚点。
- L4 新增 `opening_repetition` 检查：拦截相邻完成 shot 的同句开头或高相似开头。
- `run --resume` 增加下游 stale 检测：完成 shot 的 current revision 若早于前序 current revision，会被重新生成而不是跳过。
- 修复 `_shot_title_from_row()` 在空 `contract_json` 上访问未赋值变量的问题。

### 当前口径

`v01.c03` 当前导出不应 accept；应按新门禁重跑第 3 章或至少重写 s02/s03 后再人工 review。

### 验证

- 目标测试：`tests/test_architect_gate.py` + `tests/test_cli.py`，90 passed。
- 全量回归：522 passed, 4 warnings。
