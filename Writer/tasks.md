...
InkFlow 的 P0 单书纵向闭环已经完成：`import-baseline -> setup -> confirm-contract -> run -> scope report` 可跑通，L0 全书宪法（ARCH-4）、L1 章级节奏（ARCH-6）、三棵树架构（ARCH-12）和正文真相源锚定（ARCH-13）均已实现并归档到 `inkflow/docs/history.md`。
...
| 文档 | 位置 | 状态 |
|------|------|:---:|
| 技术设计权威 | `inkflow/docs/design.md` | ✅ v3.12 / Schema v9 |
| 实现契约 / DDL / 状态机 | `inkflow/docs/implementation-contract-v0.md` | ✅ Schema v9 |
| 三棵树与正文真相源 | `inkflow/docs/design-3tree-architecture.md` | ✅ |
| 8 层层级 | `inkflow/docs/design-8layer-hierarchy.md` | ✅ |
| 悬疑引擎 | `inkflow/docs/suspense-engine.md` | ✅ 部分实施，剩余项在待办 |
| 开发历史 | `inkflow/docs/history.md` | ✅ |
| Bug 记录 | `inkflow/docs/bugfix.md` | 本轮新增 B35/B36 |

---

## 3. 已完成任务归档

已完成项不再放在待办区，详细实现记录见 `inkflow/docs/history.md`。
