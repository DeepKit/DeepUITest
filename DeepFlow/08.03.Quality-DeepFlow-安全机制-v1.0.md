...
>
> 总体安全与数据权限框架以 `08.01.Quality-DeepFlow-安全与数据权限策略`v1.0.md` 为准，本篇聚焦于具体安全机制与实现建议。
...
| 角色 | 可执行Skill | 可访问Prompt | 可读写记忆| 可调用外部| 特殊权限 |
|------|-------------|---------------|-----------|-----------|----------|
| 引擎 | - | - | - | - | 角色管理、消息路由 |
| 督察 | 全部（Mock数据）| 全部（只读） | 全部 | 全部（Mock数据）| 断点、干扰 |
| 总指挥 | - | - | 只读 | - | 请求拒绝、资源分配 |
...
signature: string       // 模板内容的签名
  signedBy: string        // 签名者（通常是管理员或CI系统或）
  signedAt: Date
}
...
interface AdvisorOutputPolicy {
  ...
  jsonPolicy?: {
    maxDepth: number
    maxSize: number
    schema?: JSONSchema  // 必须符合JSON Schema
  }
}
```
...
- 权限检查在《DeepFlow-接口契约定义.md》的 IGuard 接口中实现
