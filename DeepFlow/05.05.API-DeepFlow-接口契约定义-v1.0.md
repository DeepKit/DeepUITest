...
**副作用声明**：`createInstance` 写入状态存储；`executeNext` 触发角色调用链
...
**副作用声明**：调用LLM（消耗token）；读取记忆（通过记录员）中

**可测试性**：所有输入/输出为结构化数据；可 Mock LLM 响应中
...
├─────────────────────────────────────────────────────────────────┤
│                                                                │
│  场景1：纯 AI 任务（理解/生成/诊断）                           │
│  ┌─────────┐                                                  │
│  │ 调度器  │──AgentTask──▶ 智囊 │──LLM调用──▶ 通信引擎 │
│  └─────────┘                                                  │
│  智囊直接调用 LLM，不经过执行层                                │
│ 场景2：纯 Skill 任务（无 AI 执行）                             │
│ 场景3：AI 增强 Skill（Skill 内部需调用 AI）                     │
│ ┌─────────┐                                                  │
│ │ 调度 ──SkillTask──→ 执行 │                            │
│ └─────────┘                                                  │
│ 执行者直接执行，不涉及智囊                                    │
...
- 本文与《DeepFlow-内部角色职能、边界与分工.md》配套，提供代码级定义
